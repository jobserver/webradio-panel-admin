app.controller('oracao',function($scope,$http,$sce,$window,$timeout,$location,$filter,$q)
{
	$scope.lista =[];
	...
	{ 
		...
		$http({
			method:'POST',
			url:$scope.instalacao+'/admin/oracao/get/'+pagina,
	...
	{		
		if ($scope.pedidoAtivo.status ==1) 
		{
			...
			$http.post($scope.instalacao+'/admin/oracao/confirmarLeitura',{'id':$scope.pedidoAtivo.id});	
		}
	};
